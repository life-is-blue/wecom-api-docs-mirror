...
| subject | string | 会议主题 |
| status | string | 会议状态： MEETING_STATE_INVALID：非法或未知状态  MEETING_STATE_INIT：待开始 MEETING_STATE_CANCELLED：已取消 MEETING_STATE_STARTED：进行中 MEETING_STATE_ENDED：已删除 MEETING_STATE_NULL：无状态（过了预定结束时间，会中无人） MEETING_STATE_RECYCLED：已回收（过了预定开始时间30天，会议号被后台回收，无法再进入） |
