...
如果配置的是小程序按钮，需要开发者的小程序接入[小程序插件](./93884.md)。
...
| chat_id_list | 是 | 使用该配置的客户群ID列表，最多支持5个。普通应用通过[获取当前客户群的群ID](./100747.md)获取群ID，营销获客类应用通过[选择客户群](#60210)获取群ID。 |
...
| chat_id_list | 使用该配置的客户群ID列表。见[客户群ID获取方法](./100747.md) |
| qr_code | 联系二维码的URL或[小程序插件](./93884.md)的URL |
...
| room_base_name | 否 | 自动建群的群名前缀，当auto_create_room为1时有效。最长40个utf8字符 |
| room_base_id | 否 | 自动建群的群起始序号，当auto_create_room为1时有效 |
| chat_id_list | 是 | 使用该配置的客户群ID列表，最多支持5个。见[客户群ID获取方法](./100747.md) |
| state | 否 | 企业自定义的state参数，用于区分不同的入群渠道。不超过30个UTF-8字符 如果有设置此参数，在调用获取客户群详情接口时会返回每个群成员对应的该参数值，详见文末[附录2](#%E9%99%84%E5%BD%952%EF%BC%9A%E8%8E%B7%E5%8F%96%E5%AE%A2%E6%88%B7%E7%BE%A4%E8%AF%A6%E6%83%85%EF%BC%8C%E8%BF%94%E5%9B%9Estate%E5%8F%82%E6%95%B0) |
