...
| monitor_status | uint32 | 告警通知状态： 0：未开启 1：已开启 |
| is_allow_call | bool | 是否允许被呼叫： true：是 false：否 |
| scheduled_status | uint32 | 预定状态： 0：未开放预定 1：开放预定 |
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| account_type | uint32 | 账号类型： 0：普通 1：专款 2：试用 |
