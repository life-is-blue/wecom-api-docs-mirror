...
**请求地址:**https://qyapi.weixin.qq.com/cgi-bin/checkin/add\_checkin\_record?access\_token=ACCESS\_TOKEN
...
| userid | 是 | 用户id |
| checkin\_time | 是 | 打卡时间。Unix时间戳 |
| location\_title | 是 | 打卡地点title，限制1024字符 |
| location\_detail | 是 | 打卡地点详情限制1024字符 |
...
| mediaids | 否 | 打卡的附件media\_id，可使用media/upload上传附件。当前最多只允许传1个 |
...
| device\_type | 是 | 打卡设备类型：1、门禁 2、考勤机（人脸识别、指纹识别） 3、其他； |
| device\_detail | 是 | 打卡设备品牌：字符串写入（限制40个字符内） |
