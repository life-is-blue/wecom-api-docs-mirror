...
| 退款渠道 | channel | string[1,16] | 否 | ORIGINAL：原路退款 BALANCE：退回到余额 OTHER_BALANCE：原账户异常退到其他余额账户 OTHER_BANKCARD：原银行卡异常退到其他银行卡 示例值： ORIGINAL |
| 退款入账账户 | user_received_account | string[1,64] | 否 | 取当前退款单的退款入账方。 退回银行卡：{银行名称}{卡类型}{卡尾号} 退回支付用户零钱：支付用户零钱 退还商户：商户基本账户、商户结算银行账户 退回支付用户零钱通：支付用户零钱通 示例值： 招商银行信用卡0403 |
...
| 用户退款金额 | payer_refund | int | 否 | 退款给用户的金额，不包含所有优惠券金额。 示例值：888 |
| 优惠退款金额 | discount_refund | int | 否 | 优惠券的退款金额，原支付单的优惠按比例退款。 示例值：888 |
