...
在文件中需要替换的位置构造转译模版，其中通讯录相关的ID转译参考[通讯录ID转译概述](#18231/%E9%80%9A%E8%AE%AF%E5%BD%95ID%E8%BD%AC%E8%AF%91%E6%A8%A1%E7%89%88%E8%AF%AD%E6%B3%95)，群聊名称和消息内容的转译模版如下：
...
* 其中 DEPARTMENT_ID 是数字类型的部门id，USERID 是[成员账号](./90307.md#userid)。
* 其中 CHATID是群聊ID，MSGID是消息ID。
* SECRET-KEY是[获取会话记录](#42660/%E8%8E%B7%E5%8F%96%E4%BC%9A%E8%AF%9D%E8%AE%B0%E5%BD%95)接口返回的这条消息对应的 encrypted_secretkey 字段进行解密得到，参考 [encrypt_secretkey 解密方式](./99961.md)。
...
| code | 是 | 从[会话展示组件获取的code](./100076.md#codec9e6905f2e10c31acbbc7cf507c88c7e)，只能使用一次，有效时间为五分钟 |
| media_id | 是 | 导出内容的模板文件media_id，可以调用[上传临时素材](./90389.md)接口获取 必须是以`file`类型上传的文件 支持Word(doc/docx)、Excel(xls/xlsx)和Txt(txt)格式的文件 |
...
> 使用[创建会话内容导出任务](#52354/%E5%88%9B%E5%BB%BA%E4%BC%9A%E8%AF%9D%E5%86%85%E5%AE%B9%E5%AF%BC%E5%87%BA%E4%BB%BB%E5%8A%A1)接口获得的jobid调用
...
| 参数 | 必须 | 说明 |
| --- | --- | --- |
| jobid | 是 | [创建会话内容导出任务](#52354/%E5%88%9B%E5%BB%BA%E4%BC%9A%E8%AF%9D%E5%86%85%E5%AE%B9%E5%AF%BC%E5%87%BA%E4%BB%BB%E5%8A%A1)接口获得的任务id |
...
| result_id | 结果id，任务处于已完成状态，且**任务返回码**（非接口返回码）为0时返回，用于在会话展示组件中展示结果。该结果只可用[会话展示组件](./100076.md#%E6%A8%A1%E6%9D%BF%E7%BB%84%E4%BB%B6)的 `ww-open-result-link` 模板组件进行展示。14天内有效。 |
