...
在用户分享网页或小程序前，先调用[setShareAttr](#setShareAttr)，将本页面的转发声明为shareticket消息。声明之后，该页面被转发之后都有shareticket属性，转发途径包括：右上角菜单、聊天附件栏页面的底部“发送”按钮、聊天附件栏或聊天工具栏调用[sendChatMessage](./94370.md)接口等。
...
带有shareticket的消息卡片，用户打开网页或小程序，开发者调用[getContext](./94321.md)时，可以得到shareticket。再根据shareticket调用[getShareInfo](#getShareInfo)，如果当前用户是消息的直接接收者，则企业微信会返回加密的encryptedData，如果不是直接接收者，则接口会返回错误。
...
| state | String | 否 | 详见[state的作用](#state%E7%9A%84%E4%BD%9C%E7%94%A8) |

### getContext

接口定义详见“[wx.qy.getContext](./94321.md)”，以下是示例代码：
...
| 参数名 | 类型 | 是否必须 | 说明 |
| --- | --- | --- | --- |
| shareTicket | String | 是 | 调用[getContext](./94321.md)时获取到的shareTicket |
...
将encryptedData解密得到的开放数据为以下 json 结构（解密算法详见“[加密数据encryptedData解密算法](#%E5%8A%A0%E5%AF%86%E6%95%B0%E6%8D%AEencryptedData%E8%A7%A3%E5%AF%86%E7%AE%97%E6%B3%95)”）：
...
| state | String | 否 | setShareAttr时传入的state参数，详见[state的作用](#state%E7%9A%84%E4%BD%9C%E7%94%A8) |
| curReceiver | String | 否 | 当前接收者的userid，对于自建应用返回的是明文userid，对于第三方应用返回的是加密的open_userid。详见[curReceiver的作用](#curReceiver%E7%9A%84%E4%BD%9C%E7%94%A8) |
...
3. 对称解密秘钥 aeskey = Base64_Decode([session_key](./91507.md)), aeskey长度为16字节。
