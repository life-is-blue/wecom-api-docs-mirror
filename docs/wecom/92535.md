...
| items.jump\_url | 否 | 点击跳转url，若不填且应用设置了主页url，则跳转到主页url，否则跳到应用会话窗口。长度不超过1024个字符 。**仅当应用主页为网页时配置生效，若应用主页为小程序，填写该字段不同的客户端平台或者版本表现可能不一致** |
...
| jump\_url | 否 | 点击跳转url。若不填且应用设置了主页url，则跳转到主页url，否则跳到应用会话窗口。**仅当应用主页为网页时配置生效，若应用主页为小程序，填写该字段不同的客户端平台或者版本表现可能不一致** |
...
| items.jump\_url | 否 | 点击跳转url，若不填且应用设置了主页url，则跳转到主页url，否则跳到应用会话窗口。**仅当应用主页为网页时配置生效，若应用主页为小程序，填写该字段不同的客户端平台或者版本表现可能不一致** |
...
| --- | --- | --- |
| url | 是 | 渲染展示的url |
| jump\_url | 否 | 点击跳转url。若不填且应用设置了主页url，则跳转到主页url，否则跳到应用会话窗口。如果enable\_webview\_click为true，则jump\_url失效，点击不再跳转。**仅当应用主页为网页时配置生效，若应用主页为小程序，填写该字段不同的客户端平台或者版本表现可能不一致** |
| pagepath | 否 | 该字段填小程序pagepath，若未设置，跳到小程序主页。**仅当应用主页为小程序时配置生效，若应用主页为网页，填写该字段不同的客户端平台或者版本表现可能不一致** |
| height | 否 | 高度。可以有两种选择：single\_row与double\_row。当为single\_row时，高度为106px（如果隐藏标题则为147px）。当为double\_row时，高度固定为171px（如果隐藏标题则为212px）。默认值为double\_row |
| hide\_title | 否 | 是否要隐藏展示了应用名称的标题部分，默认值为false |
| enable\_webview\_click | 否 | 是否开启webview内的链接跳转能力，默认值为false。注意：开启之后，会使jump\_url失效。链接跳转仅支持以下schema方式：wxwork://openurl?url=xxxx，注意url需要进行编码。参考示例：`<a href="wxwork://openurl?url=https%3A%2F%2Fwork.weixin.qq.com%2F">今日要闻</a>` |
...
**请求方式：**POST（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/agent/set\_workbench\_template?access\_token=ACCESS\_TOKEN
...
| replace\_user\_data | 否 | 是否覆盖用户工作台的数据。设置为true的时候，会覆盖企业所有用户当前设置的数据。若设置为false,则不会覆盖用户当前设置的所有数据。默认为false |
...
**请求方式：**POST（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/agent/get\_workbench\_template?access\_token=ACCESS\_TOKEN
...
**请求方式：**POST（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/agent/set\_workbench\_data?access\_token=ACCESS\_TOKEN
...
**请求方式：**POST（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/agent/batch\_set\_workbench\_data?access\_token=ACCESS\_TOKEN
...
| access\_token | 是 | 调用接口凭证 |
| agentid | 是 | 应用id |
| userid\_list | 是 | 需要设置的用户userid列表，最多1000个 |
...
**请求方式：**POST（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/agent/get\_workbench\_data?access\_token=ACCESS\_TOKEN
...
> 注：若是设置了应用的模板，并且在设置用户数据后设置，且配置了"replace\_user\_data":true，应用数据会覆盖个人数据，此时返回应用设置的数据。
...
| Event | 事件类型：switch\_workbench\_mode，切换工作台自定义模式 |
