...
商户系统获取完预支付交易会话标识（prepay\_id）后，在小程序端调用wxsdk前，需要调用本接口获取到必要的签名字段，再调起微信支付。

---

**请求方式：**POST(HTTPS)
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/miniapppay/get\_sign?access\_token=ACCESS\_TOKEN
...
| 预支付交易会话标识 | prepay\_id | string[1,128] | 是 | 小程序下单接口返回的prepay\_id参数值.仅支持下单两小时内的prepay\_id |
| 签名方式 | sign\_type | string[1,32] | 否 | 签名类型，默认为RSA，仅支持RSA。 |
...
| 参数名 | 变量 | 类型[长度限制] | 说明 |
| --- | --- | --- | --- |
| 签名值 | pay\_sign | string[1,512] | 签名，使用字段appid、timestamp、nonce、prepay\_id计算得出的签名值 |
