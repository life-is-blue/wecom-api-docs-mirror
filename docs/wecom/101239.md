...
「接收外部数据」是智能表格提供的一项开放能力，用于让外部系统通过 Webhook 协议，将数据写到智能表格中。当用户在智能表格中开启「接收外部数据」后，系统会为该表格生成唯一的 Webhook 地址，可通过标准 HTTP POST 请求 [新增智能表格记录](./60594.md) 及 [更新智能表格记录](./60595.md)。
...
![](https://wework.qpic.cn/wwpic3az/774801_t0E6rbIsT2-DCKD_1769174842/0)

入口一：智能表格文档右上角菜单 - 点击「接收外部数据」- 开始配置 - 选择目标工作表

![](https://wework.qpic.cn/wwpic3az/449094_KfleKuCDTQCLDpk_1769174859/0)
...
* 拥有 Webhook 地址的任何方均可向该工作表新增记录，一定要**保护好接收外部数据的Webhook地址**，避免泄漏！不要分享到github、博客等可被公开查阅的地方，否则坏人就可以用你的Webhook地址向工作表中写入垃圾内容。
* 若发现Webhook 地址泄露，请在智能表格「接收外部数据」设置中立即停用对应工作表的Webhook地址并重新配置新的Webhook地址。
* 支持配置推送来源 IP 白名单。启用后，仅允许在白名单内的IP向工作表新增记录。
* 智能表格支持通过 Webhook 更新已有记录，支持更新人工创建或通过其他方式导入的记录。

## 如何使用「接收外部数据」

开发者开启「接收外部数据」获取到对应工作表Webhook地址后，可以为智能表格工作表[添加记录](./60594.md)和[更新记录](./60595.md)。
