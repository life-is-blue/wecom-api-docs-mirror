...
在用户分享网页或小程序前，先调用[setShareAttr](./100508.md)，将本页面的转发声明为shareticket消息。声明之后，该页面被转发之后都有shareticket属性，转发途径包括：右上角菜单、调用[shareAppMessage](./100501.md#%E8%87%AA%E5%AE%9A%E4%B9%89%E8%BD%AC%E5%8F%91%E5%88%B0%E4%BC%9A%E8%AF%9D)接口、聊天附件栏页面的底部“发送”按钮、聊天附件栏或聊天工具栏调用[sendChatMessage](./100749.md)接口等。
...
带有shareticket的消息卡片，用户打开网页或小程序，开发者调用[getContext](./100509.md)时，可以得到shareticket。再根据shareticket调用[getShareInfo](./100510.md)，如果当前用户是消息的直接接收者，则企业微信会返回加密的encryptedData，如果不是直接接收者，则接口会返回错误。
...
3. 对称解密秘钥 aeskey = Base64_Decode([应用身份使用的ticket](./90245.md##%E8%8E%B7%E5%8F%96%E5%BA%94%E7%94%A8-jsapi-ticket)), aeskey长度为16字节。
