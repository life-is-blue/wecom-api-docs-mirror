...
* 在__非企业微信内置浏览器__中使用时，页面需要使用 [Web 登录组件](https://developer.work.weixin.qq.com/document/path/98152)进行登录，必须使用企业微信登录组件方式（非构造登录链接），且使用会话展示组件的域名必须与嵌入 Web 登录组件的域名完全一致；
...
如果被包裹的 `ww-open-message` 组件的内容不是__图片__、__视频__，不会产生任何影响。
...
| secret-key | string |  | 是 | 消息密钥，对会话记录中的 encrypted_secretkey 字段进行解密得到，参考 [encrypt_secretkey 解密方式](./51799.md) |
