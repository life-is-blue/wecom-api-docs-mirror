...
* 准备好一个[自建应用](./53575.md)或者[第三方应用或代开发应用](./53518.md)，应用保证 [agentConfig](https://developer.work.weixin.qq.com/document/path/94325#90002/90144/94325) 签名可以正常工作；
* 在\_\_非企业微信内置浏览器\_\_中使用时，页面需要使用 [Web 登录组件](https://developer.work.weixin.qq.com/document/path/98152)进行登录，必须使用企业微信登录组件方式（非构造登录链接），且使用会话展示组件的域名必须与嵌入 Web 登录组件的域名完全一致；
...
在初始化阶段，页面需要调用 `@wecom/jssdk` 提供的 [ww.register](./46258.md) 接口初始化应用信息：
...
如果被包裹的 `ww-open-message` 组件的内容不是\_\_图片\_\_、\_\_视频\_\_，不会产生任何影响。
...
| externalUserName | 若是企业客户填：externalUserId  若是客户群的外部成员填：chatid/externalUserId，例如：wraaaabbbb/wmccccdddd；若当前企业为K12教育行业，externalUserId在家校通讯录中，则会转译为家长或者学生的名称。若非家长或者学生，则展示“非学校家长” | 客户名称。若员工对客户有设置备注名，则展示“备注名(名称）”；若名称和备注名一致时仅展示“备注名”；多个员工给客户有备注的，展示最早添加的员工给客户的备注 |
| externalUserAvatar | 若是企业客户填：externalUserId  若是客户群的外部成员填：chatid/externalUserId，例如：wraaaabbbb/wmccccdddd；若当前企业为K12教育行业，externalUserId在家校通讯录中，则会转译为家长或者学生的头像。 | 客户头像 |
| chatName | chatid | 内部群或客户群名 |
| fileName | 文件/文档url | 文件/文档标题。异常情况： - url错误，展示「链接错误」 - 非本企业的文件/文档，且用户无浏览权限，展示「暂无权限查看该文档/文件」 - 文件url，且应用无“微盘”权限，展示「暂无微盘应用权限」 - 文档url，且应用无“文档”权限，展示「暂无文档应用权限」 |
...
| secret-key | string |  | 是 | 消息密钥，对会话记录中的 encrypted\_secretkey 字段进行解密得到，参考 [encrypt\_secretkey 解密方式](./51799.md) |
...
| message-id | string |  | 是 | 消息ID（与 result 结果相关的任一消息） |
...
关于文件下载 url：先调用“[异步通讯录id转译](./17813.md)”接口提交转译任务，然后通过“[获取异步任务结果](./18011.md)”获取最终下载文件的url，注意该url仅能在有用户登录态的浏览器打开以下载文件，详情参见“[通讯录ID转译](#90001/90143/91845)”。
