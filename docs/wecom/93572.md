...
为了防止滥用，每次群发最多可选2000个最近活跃的客户群，同一个成员每日向一个客户群最多可群发一条消息，可在企业微信-群发助手-设置群发规则中调整为每周接收七条或每月可接收条数为当月天数。

> **调用说明：**
> 文本最多支持传入4000个字
> 附件最多支持传入9个，类型支持图片(“image”)，视频(“video”)，图文(“link”)，小程序(“miniprogram”)和文件(“file”)
> 支持传入文本和多附件的能力，仅在企业微信3.1.6及以后版本支持（mac端暂不支持）（[查看旧版本接口调用方式](#29622)）。微信客户端和微信开发者工具都不支持该接口。
...
> \* text和attachments不能同时为空（注：若text和attachments同时为空，接口使用老的方式[wx.qy.shareToExternalChat](./93572.md)方式传参也可以兼容。）
