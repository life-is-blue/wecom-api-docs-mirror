...
| 交易状态 | trade_state | string[1,32] | 是 | 交易状态，枚举值： SUCCESS：支付成功 REFUND：转入退款 NOTPAY：未支付 CLOSED：已关闭 REVOKED：已撤销（付款码支付 USERPAYING：用户支付中（付款码支付） PAYERROR：支付失败(其他原因，如银行返回失败) ACCEPT：已接收，等待扣款 示例值：SUCCESS |
...
| 优惠范围 | promotion_detail.scope | string[1,32] | 否 | GLOBAL：全场代金券 SINGLE：单品优惠 |
| 优惠类型 | promotion_detail.type | string[1,32] | 否 | CASH：充值型代金券 NOCASH：免充值型代金券 示例值：CASH |
