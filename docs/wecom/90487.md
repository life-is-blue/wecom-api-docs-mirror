...
![](http://p.qpic.cn/pic_wework/3903686901/0107e00df75b83aac9934d61515996a1ddc7c93c146cc5b6/0)
...
发消息api见[发送应用消息](#10167)，可以看到其实就是一个post请求
...
参考[开始开发](./90306.md)，access_token是应用调用api的凭证，由 [corpid](./90307.md#corpid)和[corpsecret](./90307.md#secret)换取。
...
参考[发送应用消息](#10167)，可以发送文本、图片、视频等多种类型的应用消息，这里以最简单的文本消息为例：（注意修改touser、agentid为自己想要的接收者userid列表、应用id）
...
实际上，企业微信提供了一套更方便的模拟工具，见[开发者工具](./90312.md)的“接口调试工具”。

### debug模式调用接口

在开发过程中，可能由于你调用的参数有问题，我们的接口会返回errcode, 此时你可以在“[全局错误码说明](#10649)”查阅相应的错误原因。有时候可能根据错误码说明你仍然不知道自己的参数在哪里出错，这时候你可以在接口请求url里加上debug=1参数（暂未支持微盘相关接口），之后从接口返回的errmsg复制出hint值，再用以下工具进行查询，我们会返回你请求的完整参数（包括header与body）。
