...
| attendees.tmp_external_user | 会中参会的外部联系人 `注意：微信入会的用户不会返回` |
| attendees.tmp_external_user.tmp_external_userid | 会中入会的外部用户临时id。同一个用户在不同会议中返回的该id不一致。 可进一步通过[tmp_external_userid的转换](./98729.md)接口转换成external_userid，方便识别外部参会人身份。 |
