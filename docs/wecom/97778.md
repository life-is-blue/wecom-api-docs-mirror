...
| corp\_internal\_auth | uint32 | 否 | 企业内成员主动查看文档后获得的权限类型 1:只读 2:读写, 有值则覆盖 |
| enable\_corp\_external | bool | 否 | 是否允许企业外成员浏览文档, 有值则覆盖 |
| corp\_external\_auth | uint32 | 否 | 企业外成员主浏览文档后获得的权限类型 1:只读 2:读写, 有值则覆盖 |
...
| auth | uint32 | 否 | 文档特定部门权限 1:只读 2:读写 |
