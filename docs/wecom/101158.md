...
| ver | uint32 | 否 | 版本号 |
| filter_spec | object([FilterSpec](./101155.md#filterspec)) | 否 | 过滤设置，不支持和sort一起使用 |
...
| --- | --- | --- |
| user_id | string | 成员ID |
| tmp_external_userid | string | 外部用户临时ID，同一个用户在不同的智能表中返回的该ID不一致。可进一步通过[tmp_external_userid的转换](./98729.md)接口转换成external_userid，方便识别外部用户的身份。 |
| agentid | uint32 | 应用ID |
...
| --- | --- | --- |
| id | string | 选项ID |
| style | uint32 | 选项颜色[(Style)](./101157.md#style) |
| text | string | 选项内容 |
