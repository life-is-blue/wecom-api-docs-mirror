...
| 参数 | 是否必填 | 说明 |
| --- | --- | --- |
| eventtype | 是 | 事件类型，例如template\_card\_event |
| eventtype\_name | 否 | 具体的事件结构体。例如当eventtype为template\_card\_event时，eventtype\_name字段名为template\_card\_event。具体可参考[模板卡片事件](#%E6%A8%A1%E6%9D%BF%E5%8D%A1%E7%89%87%E4%BA%8B%E4%BB%B6) |
...
| 参数 | 说明 |
| --- | --- |
| eventtype | 事件类型，此处固定为enter\_chat |
...
其中，eventtype固定为template\_card\_event。对应结构体TemplateCardEvent。

**参数说明：**

| 参数 | 说明 |
| --- | --- |
| response\_url | 支持主动回复消息的[url](./59947.md) |
...
| task\_id | 是 | 用户点击的交互模版卡片的task\_id |
| selected\_items | 否 | 用户点击提交的选择框数据，参考SeletedItem结构说明 |

**参考SeletedItem结构说明：**

| 参数 | 是否必填 | 说明 |
| --- | --- | --- |
| question\_key | 用户点击提交的选择框的key值 |  |
...
| 参数 | 说明 |
| --- | --- |
| cardtype | 事件类型，此处固定为button\_interaction |
| selected\_items | 下拉式的选择器选择的数据 |
...
| selected\_items | 用户点击提交的投票选择框数据 |
...
| selected\_items | 用户点击提交的下拉菜单选择框列表数据 |
...
| 参数 | 说明 |
| --- | --- |
| eventtype | 事件类型，此处固定为feedback\_event |
| feedback\_event.id | [回复消息](#59068/%E6%B5%81%E5%BC%8F%E6%B6%88%E6%81%AF%E5%9B%9E%E5%A4%8D)设置的反馈id |
| feedback\_event.type | 反馈的类型： 1：准确   2：不准确  3：取消准确/不准确 |
| feedback\_event.content | 用户输入的反馈内容，仅不准确反馈类型支持返回 |
| feedback\_event.inaccurate\_reason\_list | 用户选择的负反馈原因列表，仅不准确反馈类型支持返回。负反馈原因有：  1: 与问题无关   2: 内容不完整   3: 内容有错误   4: 数据分析错误 |
