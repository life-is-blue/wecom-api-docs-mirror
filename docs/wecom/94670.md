...
| text.menu_id | string | 客户点击[菜单消息](#25217/%E8%8F%9C%E5%8D%95%E6%B6%88%E6%81%AF)，触发的回复消息中附带的菜单ID |
...
| event.welcome_code | string | 如果满足发送欢迎语条件（条件为：**用户在过去48小时里未收过欢迎语，且未向客服发过消息**），会返回该字段。 可用该welcome_code调用[发送事件响应消息](./95122.md)接口给客户发送欢迎语。 |
...
| event.msg_code | string | 用于发送事件响应消息的code，仅change_type为1和3时，会返回该字段。 可用该msg_code调用[发送事件响应消息](./95122.md)接口给客户发送回复语或结束语。 |
