...
| tmp\_external\_userid | string | 外部用户临时id，匿名填写不返回，同一个用户在不同的收集表中返回的该id不一致。 可进一步通过[tmp\_external\_userid的转换](./98729.md)接口转换成外部联系人的external\_userid，方便识别外部填写人的身份。 |
