...
我们提供了一套专区程序的示例代码，开发者可以在示例源码的基础上进行开发，再根据[镜像文件配置指引](./53065.md)制作镜像。开发者也可以先下载我们制作好的示例镜像，根据[镜像文件配置指引](./53065.md)在管理端上传配置镜像。示例源码和示例镜像可通过[这里下载](./55652.md)。
以下是该镜像的启动命令与程序配置说明。
...
> `func`固定为`sync_msg`，此时`func_req`为[获取会话记录](./53341.md)对应的输入协议
...
> 调用[应用同步调用专区程序](./53295.md)接口时，外层`notify_id`传入[应用接收专区通知](./53360.md)回调的`NotifyId`字段
...
ability\_id: invoke\_sync\_msg

> ability\_id格式：invoke\_$API\_NAME$
> 注意：Java程序示例对透传请求的处理逻辑为：**将传入的能力ID截断“invoke\_”前缀后，直接用于调用sdk。**开发者如果需要自定义透传请求的能力ID，请额外添加自定义能力ID与sdk接口名的映射，并保证接口名合法

输入协议：
注意：Java程序示例的默认输入协议中，request\_data内无需包裹一层input对象
...
ability\_id：get\_callback\_data
...
| event\_type | 事件类型，各回调事件的共有字段 |
| timestamp | 时间戳，各回调事件的共有字段 |
| chat\_archive\_audit\_approved | [客户同意进行聊天内容存档事件回调](https://developer.work.weixin.qq.com/document/path/99993) |
| conversation\_new\_message | [产生会话回调通知](https://developer.work.weixin.qq.com/document/path/99994) |
| hit\_keyword | [命中关键词规则通知](https://developer.work.weixin.qq.com/document/path/99995) |
| auth\_knowledge\_base | [知识集管理回调](https://developer.work.weixin.qq.com/document/path/99996) |
| chat\_archive\_export\_finished | [会话内容导出完成通知](https://developer.work.weixin.qq.com/document/path/99997) |
...
| -10008 | sdk内部错误 | 密钥版本不匹配。如果是调试模式，可能是access\_token过期导致未正确返回 |
...
> ability\_id格式：invoke\_$API\_NAME$
> 注意：C++程序示例对透传请求的处理逻辑为：**将传入的能力ID截断“invoke\_”前缀后，直接用于调用sdk。**开发者如果需要自定义透传请求的能力ID，请额外添加自定义能力ID与sdk接口名的映射，并保证接口名合法

输入协议：
注意：C++程序示例的默认输入协议中，request\_data内无需包裹一层input对象
...
程序审核通过并且部署成功后，可使用授权了数据与智能专区权限的应用使用接口[应用同步调用程序](./53295.md)。
...
2. 专区程序接收回调后，需要企业应用来拉取数据时，调用spec\_notify\_app通知企业应用（下图2.1），这里会给专区程序返回一份notify id，并向企业应用的回调地址发送一份包含**相同的notify id**的回调（下图2.2），专区程序需要将该notify id和待应用查询的数据作关联
   * 详见：[专区通知应用](https://developer.work.weixin.qq.com/document/53408)
3. 应用收到企微专区通道的回调后（下图2.2），请求专区程序（下图3.1），获取专区程序暂存的token
   * 请求的ability id为get\_callback\_data
   * 请求需要捎带notify id
   * 请求为同步调用
4. 应用拿到token后，可调用专区程序获取会话记录
   * 请求的ability id为invoke\_sync\_msg
   * 请求的request\_data中捎带token
