...
我们提供了一套专区程序的示例代码，开发者可以在示例源码的基础上进行开发，再根据[镜像文件配置指引](./53065.md)制作镜像。开发者也可以先下载我们制作好的示例镜像，根据[镜像文件配置指引](./53065.md)在管理端上传配置镜像。示例源码和示例镜像可通过[这里下载](./100249.md)。
以下是该镜像的启动命令与程序配置说明。
...
> `func`固定为`sync_msg`，此时`func_req`为[获取会话记录](./99968.md)对应的输入协议
...
> 调用[应用同步调用专区程序](./99965.md)接口时，外层`notify_id`传入[应用接收专区通知](./99998.md)回调的`NotifyId`字段
...
程序审核通过并且部署成功后，可使用授权了数据与智能专区权限的应用使用接口[应用同步调用程序](./99965.md)。
