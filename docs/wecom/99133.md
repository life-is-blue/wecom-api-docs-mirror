...
| update_items | 否 | 需要更新、增加或清空单个字段的内容，参考[更新字段说明](#update_items)。 有一些字段不支持更新，参考[不支持更新字段表](#forbidden_fields)。 这个字段和remove_items、insert_items字段不能全部为空 |
| remove_items | 否 | 可重复的字段组(参考[可重复字段组列表](#multigroup))中需要整组字段进行删除的字段组，参考[删除字段说明](#remove_items)。 这个字段和update_items、insert_items字段不能全部为空 |
| insert_items | 否 | 可重复的字段组(参考[可重复字段组列表](#multigroup))中需要增加一组字段的字段组，参考[增加字段说明](#addfield_items)。 这个字段和update_items、remove_items字段不能全部为空 |
...
| value_xxxxx | 否 | 需要更新、增加或清空的员工信息字段内容。 根据员工信息字段的类型，需要填写的这个字段的名称和类型也不同，参考[字段值类型对照表](#value_type)。 除了对应的字段外，在其他字段填写的内容将被忽略。 |
...
| item | 否 | 列表，需要增加的字段内容； 填写要求与[更新字段说明](#update_items)相同，但sub_idx字段的内容将被忽略； 没有找到对应字段id的字段内容将被忽略。 |
...
| value_mobile | 字符串，表示电话号码 这个字段如果不填/填写空串，则视为整个电话号码字段传入为空 |
