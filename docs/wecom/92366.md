...
| file.media_id | 文件id，可以通过[素材管理](#10112)接口获得。 **注意：设置接口若使用[异步上传临时素材](#42044)拿到的media_id，此时就会返回同类型的media_id** |
| video.media_id | 视频媒体文件id，可以通过[素材管理](#10112)接口获得。 **注意：设置接口若使用[异步上传临时素材](#42044)拿到的media_id，此时就会返回同类型的media_id** |
