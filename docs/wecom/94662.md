...
**请求地址**: https://qyapi.weixin.qq.com/cgi-bin/kf/account/add?access\_token=ACCESS\_TOKEN
...
| name | 是 | string | 客服名称 不多于16个字符 |
| media\_id | 是 | string | 客服头像临时素材。可以调用[上传临时素材](./10112.md)接口获取。 不多于128个字节 |
...
| 应用类型 | 权限要求 |
| --- | --- |
| 自建应用 | 配置到「 [微信客服- 可调用接口的应用](./94638.md#%E5%A6%82%E4%BD%95%E5%BC%80%E5%90%AFAPI)」中，且在管理后台「通过API管理会话消息」-「企业内部开发」对应的自建应用的「可管理的客服账号」处，配置至少一个客服账号 |
