...
**请求方式：**GET（**HTTPS**）
**请求地址：**https://qyapi.weixin.qq.com/cgi-bin/batch/getresult?access\_token=ACCESS\_TOKEN&jobid=JOBID
...
| type | 操作类型，字节串，目前分别有：1. sync\_user(增量更新成员) 2. replace\_user(全量覆盖成员)3. replace\_party(全量覆盖部门) |
| total | 任务运行总条数 |
| percentage | 目前运行百分比，当任务完成时为100 |
| result | 详细的处理结果，具体格式参考下面说明。当任务完成后此字段有效 |

**result结构：type为sync\_user、replace\_user时：**
